...
| **W3 `webrtc-ice`** | canonical TA fixtures (dual-symmetric-NAT→relay, candidate-flap, `NetworkId` change→restart); timeline fuzz smoke + JVM deep-run with shrinker; ICE invariants in the fuzz set; **mDNS both ways over the vnet** — one `MdnsEndpoint` advertises a `<uuid>.local`, another that knows only the name resolves it over a link-local group, with only the 5353 bind/join substituted (`MdnsEndpointTest`, `MdnsResponderTest`); the production `NetworkMonitor` (`SystemNetworkMonitorTest`) — **reactivity asserted as a virtual-clock delta** so a regression to polling fails by orders of magnitude rather than passing slowly, plus callback-burst coalescing, registration/teardown on collect-and-cancel, and failed-probe handling against a scripted `InterfaceEnumerator`; `jvmTest`/`nativeTest` then pin the **real** OS interface table and `detection == PlatformSignalled` on the target running them |
