...
and the full common suite — 611 tests — now runs there per PR. Pinned at **socket 4.4.1 + buffer
6.29.0**, both the latest published. socket
...
it is unchanged from 4.1.0 through 4.4.1. 4.4.1 itself is one `socket-quic` commit over 4.4.0 (#346),
a module this repo depends on at no coordinate, so it carries nothing we consume.
...
built against an older one is safe. **socket-udp 4.4.1's POM declares buffer 6.28.0** — the one version
...
- **Platforms:** Node needs blocking raw-ECDH plus a shipped binder (#133). tvOS/watchOS establish as of
  ...
  - `watchosArm64` (**arm64_32**, 32-bit pointers) — blocked in **buffer**. buffer PR #342 **declined**
    ...
    on Central at 6.29.0; `socket-udp-watchosarm64` publishes fine at 4.4.1, so socket is not the blocker
    here.
  - `watchosDeviceArm64` (64-bit device — Series 9 / Ultra) — blocked in **socket**, and our own matrix
    ...
    `socket-udp-watchosdevicearm64` is a 404 at 4.2.0 **and still at 4.4.1** (re-checked against Central
    when this repo took each), so this one needs a socket PR plus a target registration here — a plain
    socket bump will not deliver it, which taking 4.4.0 and then 4.4.1 has now demonstrated twice rather
    than predicted.
