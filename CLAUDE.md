...
and `buffer-codec` — never `buffer-crypto` — so its POM can pin buffer *core* low and nothing else. The
upstream bump was hygiene, not a live break. **The rule that is load-bearing is about OUR pin**, because
...
- **Platforms:** Node needs blocking raw-ECDH plus a shipped binder (#133). tvOS/watchOS establish as of
  ...
  - `watchosDeviceArm64` (64-bit device — Series 9 / Ultra) — blocked in **socket**, and our own matrix
    ...
    socket bump will not deliver it, which taking 4.4.0, 4.4.1 and now 4.5.0 has demonstrated three times
