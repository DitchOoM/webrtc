...
| resolved 2026-07-16 | RFC §11.3 — DTLS version → **both 1.2 and 1.3 via BoringSSL config; negotiate 1.3-down-to-1.2, interop-test at 1.2** | We do not hand-roll the record/handshake layer — BoringSSL owns it, so supporting both versions is a `SSL_CTX_set_min_proto_version(DTLS1_2_VERSION)` / `set_max_proto_version(DTLS1_3_VERSION)` config choice, not extra code. Min is pinned at **DTLS 1.2** because that is what libwebrtc/Chrome and Pion actually speak on the wire today (Chrome shipped DTLS 1.3 only recently; the safe interop floor is 1.2); max is left at 1.3 so two of our own stacks negotiate up. The W4-native path links buffer-crypto's BoringSSL commit `63893acb` (API 42) whose `ssl.h` exposes both `DTLS1_2_VERSION`+`DTLS1_3_VERSION`, so both are live now. **Caveat (cross-platform, see the W4-sequencing row):** the future JVM/Android/Apple backends consume `boringssl-kmp`, whose canonical pin is quiche-anchored `44b3df6f` = **BoringSSL API 21 = DTLS 1.2 only** — so the *portable* floor stays 1.2 (fine: no WebRTC feature is gated on 1.3; media protection is SRTP). The W4 exit fixture handshakes our-stack↔our-stack on native (may pick 1.3); the W7 interop matrix (Pion/Chrome) exercises the 1.2 floor. No DTLS 1.0/1.1 (RFC §10 non-goal). |
