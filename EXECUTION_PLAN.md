...
| resolved 2026-07-15, revisited 2026-07-28 (#88 — the responder shipped: see below) | RFC §11.4 — mDNS → **resolve-only in W3; responder deferred** (behind a capability flag) | The gathering-side responder is multicast platform work (a `224.0.0.251:5353` listener per interface); it is not on the critical path for *reaching* peers. A browser peer that advertises a `.local` srflx-masking candidate must be *resolved* to its host IP for us to send checks, so resolve-only is mandatory in W3; advertising our own `.local` candidates (the responder) buys only privacy and is deferred behind a flag until a harness lane needs it. **Shipped in #88**, on exactly that shape: `MdnsAdvertisePolicy` is the flag, it is opt-in (a `commonMain` default cannot construct a responder, and a name nothing answers costs the peer the candidate), and the whole responder — codec, RFC 6762 §6 policy, dispatch loop — is `commonMain` over the vnet, with only the 5353 bind/join at the platform edge. mDNS resolution rides the injected gathering seam (a `MdnsResolver` interface, deterministic stub in tests), never a hardwired multicast socket in the core. |
...
> generation, so its session does not survive (measured in `test-harness/README.md`); media (RTP/SRTP) is
> **P2**, untouched. The dcSCTP subset
