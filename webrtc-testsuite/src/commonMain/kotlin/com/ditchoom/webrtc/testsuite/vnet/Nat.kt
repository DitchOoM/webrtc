...
/**
 * How a NAT assigns the **external mapping** for an outbound datagram (RFC 4787 §4.1). A cone NAT
 * reuses one external port for every destination (so a srflx address learned via STUN is reachable by
 * anyone), whereas a symmetric NAT allocates a fresh port per destination (so a srflx address is
 * useless to a third party — the reason symmetric↔symmetric can only meet on a relay).
 */
...
/**
 * Which return datagrams a NAT lets back in through an existing mapping (RFC 4787 §5). Orthogonal to
 * [Mapping]: it decides whether an inbound packet from a remote endpoint is delivered, based on whether
 * — and how specifically — the host has previously sent out to that remote.
 */
 ...
 * four canonical profiles are the named constants — the classic RFC 3489 cone/symmetric taxonomy in
 * RFC 4787's precise two-axis vocabulary, which is what actually determines ICE outcomes.
 */
 ...
 * a monotonic counter (real NATs randomize; the vnet wants replay). Not thread-safe by design — the
 * vnet is single-threaded under `runTest`.
 */
internal class NatBox(
     ...
     * null if no such mapping exists or the [Filtering] rule rejects [remote]. Rejection is a silent
     * drop, exactly as a real NAT discards it.
     */
    fun mapInbound(
...
/**
 * The internetwork [Fabric] that layers a set of [NatBox]es over a flat public segment:
 *
 ...
 *  - **cross-private** to a *different* segment's private address → **unreachable** (dropped): what
 *    forces symmetric↔symmetric peers onto a relay;
